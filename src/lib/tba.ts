...
export const tba = {
  async fetchEvent(eventKey: string): Promise<TBAEvent> {
    ...
    logger.debug('[tba.fetchEvent] request', {
    ...
    logger.debug('[tba.fetchEvent] success', {
      normalizedEventKey,
  ...
  async fetchTeams(eventKey: string): Promise<TBATeam[]> {
    const normalizedEventKey = eventKey.trim().toLowerCase();
    logger.debug('[tba.fetchTeams] request', {
      originalEventKey: eventKey,
      normalizedEventKey,
    });
    ...
    logger.debug('[tba.fetchTeams] success', {
      normalizedEventKey,
  ...
  async fetchMatches(eventKey: string): Promise<TBAMatch[]> {
    const normalizedEventKey = eventKey.trim().toLowerCase();
    logger.debug('[tba.fetchMatches] request', {
      originalEventKey: eventKey,
      normalizedEventKey,
    });
    ...
    logger.debug('[tba.fetchMatches] success', {
      normalizedEventKey,
  ...
  async fetchRankings(eventKey: string): Promise<TBARankings> {
    const normalizedEventKey = eventKey.trim().toLowerCase();
    logger.debug('[tba.fetchRankings] request', {
      originalEventKey: eventKey,
      normalizedEventKey,
    });
    ...
    logger.debug('[tba.fetchRankings] success', {
      normalizedEventKey,
  ...
  async fetchTeamMatchesByYear(teamNumber: number, year: number): Promise<TBAMatch[]> {
    ...
    logger.debug('[tba.fetchTeamMatchesByYear] request', { teamNumber, year });
    ...
    logger.debug('[tba.fetchTeamMatchesByYear] success', {
      teamNumber,
      year,
      matchCount: Array.isArray(matches) ? matches.length : 0,
    });
  ...
  async fetchMatchDetail(matchKey: string): Promise<TBAMatchDetail> {
    ...
    logger.debug('[tba.fetchMatchDetail] request', {
    ...
    logger.debug('[tba.fetchMatchDetail] success', {
      normalizedMatchKey,
  ...
  getTeams(): TBATeam[] {
    ...
    logger.debug('[tba.getTeams] cache', { teamCount: teams.length });
    return teams;
  },
